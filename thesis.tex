...
\chapter{Method}
\label{chap:method}
As was discussed in Chapter \ref{chap:star}, the majority of artists uses professional (often commercial) 3D software. We want to improve their workflow by using custom real-time renderer.
...
\section{Motivation}
Off-line rendering takes some time to finish. When artist is working, the rendering step is holding them back. It kicks them out of the flow. Because of this we want to

\section{Reasons for Using Real-time Renderer}
Modern 3D programs use mostly polygon meshes as the representation of 3D scenes. Meshes are sets of vertices, edges and triangles used to model real-life objects. For molecular data and scenes, meshes are not the best representation. A single atom is in our case simplified by a single sphere of a certain radius. Approximation of a sphere by using meshes is, unfortunately, not very good in terms of req. Very detailed mesh (consisting of thousands of triangles) must be used to create an effect of smooth spherical object.

\section{Using Shared Memory}
We use shared memory as a way of communication between our two programs. It has been naturaly the choice for this ... Are there any alternatives???

\section{Discussion of Benefits}
...
Second demonstration: blood-like dataset. Proteins of 5 different types (1atu, 1smd, 2tsc, 2hiu, 2rcj) have been imported using Molecular Maya plugin and low resolution mesh has been generated. Then, using a custom script written in Python, position and rotation of each instance has been determined randomly.
