...
Shared memory is a segment of system memory which can be accessed by multiple programs. It is used as an efficient way to establish communication between separately running processes. It is typically located in random-access-memory (RAM) although it can be swapped onto disk.

% TODO: swaping onto disk, unix, named file mapping in windows

There are several libraries that would enable us to access shared memory. First option is to use operating system API calls. Another way would be to use boost

% [\textbf{maya api}]
\section{Maya API}
Maya actually provides two APIs - one in Python and one in C++. In addition to that, there is also third way how additional functionality can be implemented - MEL scripting language.
